 ...
 *
 *     Version 3: Added vanilla Device (MCHC)
 ...
 *
 *     Version 4: Removed Method (_STA) from Device (MATH) to emulate iMac15,1
 ...
 *
 *     Version 5: Removed simple Device (B0D4)
 ...
 *
 *     Version 6: Removed Device (PS2K) (PS2M) and reference(s)
 *                Renamed Device (PXSX) to Device (LAN1) (LAN2) (ARPT) respectively
 *
 *     Version 7: Removed simple Device (NFC)
 ...
 *
 *     Version 8: Removed Device (RP06) and reference(s)
 *                Removed Device (RP07) and reference(s)
 *                Removed Device (RP08) and reference(s)
 *                Removed Device (CDPT) (CRGB) in Scope (SSP4)
 *
 * Enabled deep sleep and deep idle properties: per Pike R. Alpha, we can to obtain the following property:
 *                IOService:/AppleACPIPlatformExpert/IOPMrootDomain/IOPMDeepIdleSupported=True
 *
 *     Version 9: Renamed Device (SAT0) to Device (SATA)
 ...
 *
 * Regarding any sleep issues: For each device in pmset -g output, GPRW should return (0x09, 0x04)
 * However, in more recent MacOS X systems, it is better to remove Method (_PRW) completely.
 *
 *                Final patches based on other MSI motherboards
 *
 *    Version 10: Removed Method (_L00) from Scope (_GPE) related to Lid Switch Events
 *                Removed Method (_PS0) (_PS3) from Device (XHC)
 *                Removed Method (_PS0) (_PS2) (_PS3) from Device (RHUB) of (XHC)
 *                Removed debug Method (ADBG) everywhere
 *
 * WARNING: Do not rename or remove any Device (PEG0) (PEG1) (PEG2) or macOS will not boot!
 *
 */
...
//  External (PS3X, MethodObj)
    ...
    Name (MCKN, "MSI Z97i-AC fixes by MacKonsti")
    Name (SP1O, 0x4E)
    Name (IO1B, Zero)
    Name (IO1L, Zero)
    Name (IO2B, 0x0A00)
...
//                  }
                ...
                Device (PMCR)  // Added Vanilla Device
                {
                ...
                Method (SIOW, 1, NotSerialized)
                {
                    ...
                    ^SIO1.ENFG (0x0A)
                    ^SIO1.RGE0 &= 0x9F
                 // ^SIO1.RGE0 = ^SIO1.RGE0  // Fixed Compiler Warning
                    ^SIO1.RGE4 &= 0xFB
                 // ^SIO1.RGE4 = ^SIO1.RGE4  // Fixed Compiler Warning
                    ^SIO1.RGE6 &= 0x7F
                 // ^SIO1.RGE6 = ^SIO1.RGE6  // Fixed Compiler Warning
                    ^SIO1.EXFG ()
                }
...
//          }
        ...
        Scope (\_SB)  // Deep Sleep Properties (IOPMDeepIdleSupported)
        {
        ...
        Scope (\_GPE)  // Deep Sleep Properties (IOPMDeepIdleSupported)
        {
    ...
    Name (PNVL, 0xE4)  // Fixed Hexadecimal Format
...
//      }
        ...
        Device (EHC1)  // Intel Corporation 9 Series Chipset Family USB EHCI Controller #1 [8086:8ca6]
        {
...
//          }
        ...
        Device (EHC2)  // Intel Corporation 9 Series Chipset Family USB EHCI Controller #2 [8086:8cad]
        {
...
//          }
        ...
        Device (SATA)  // Intel Corporation 9 Series Chipset Family SATA Controller [AHCI Mode] [8086:8c82]
        {
...
//          Method (_STA, 0, Serialized)  // _STA: Status
//          {
//              Switch (ToInteger (BID))  // Fixed Compiler Warning
//              {
//                  Case (0x30)
//                  {
//                      Return (Zero)
...
//  }
    ...
    Method (_PTS, 1, NotSerialized)  // _PTS: Prepare To Sleep
    {
        ...
        P8XH (Zero, Arg0)
        PTS (Arg0)
//      If ((((((Arg0 == 0x04) || (Arg0 == 0x05)) || (Arg0 == 0x03)) && (BID == 0x31)) && (RTD3 == One)))
//      {
//          ADBG ("Wilsonbeach")
//          If (CondRefOf (\_SB.PCI0.RP06.SCLK))
//          {
//              If (((RP6D != One) && ((((RCG0 & 0x40) && !(RCG0 & 0x0200)) && (RCG0 & 0x0100)) || ((!(RCG0 & 0x40) && (RCG0 & 0x0200)) && (SATD == One)))))
//              {
...
//      }
    ...
    {
        // In older versions of MacOS, after resuming from S3 suspend
        // (Sleep, or suspend to RAM) the DSDT method (_WAK) is called
        // with garbage in Arg0 instead of having the value representing
        // the sleep state being left (1-5).
        //
        // This causes problems for code in DSDT (_WAK) method which
        // expects and checks against Arg0==3 for wake from sleep to
        // turn on various devices or restore internal state to a
        // working state. This condition at the very beginning of the
        // method forces an out-of-range Arg0 to 0x03.

        If (((Arg0 < 0x01) || (Arg0 > 0x05)))
        {
            Arg0 = 0x03
        }
...
//  }
    ...
    Scope (_SB.PCI0)
    {
        Device (HECI)  // Intel Corporation 9 Series Chipset Family ME Interface [8086:8cba]
        {
    ...
    Scope (_SB)
    {
        Device (PTMD)
        {
            ...
            Method (GXDV, 1, NotSerialized)
            {
                ...
                OperationRegion (PRFA, SystemMemory, 0xD97E1018, 0xF0)  // Fixed Hexadecimal Format
                ...
                Name (RPKG, Package (0x02){})
                Name (XMPT, Buffer (0x78){})  // Fixed Hexadecimal Format
                If ((Arg0 == One))
                {
...
//              }
                ...
                Return (Zero)  // Force Device Disable
            }
